...
def decode_yolox_predictions(predictions, conf_threshold=0.5, nms_iou_threshold=0.4):
    """Decodes anchor-free YOLOX predictions on the shared single detection grid."""
    bsz, channels, height, time_steps = predictions.shape
    if channels != 5:
        raise ValueError(f"YOLOX predictions must have 5 channels, received {channels}")
    ...
    for batch_idx in range(bsz):
        ...
        x_offset = pred_x[batch_idx, grid_y, grid_x]
        y_offset = pred_y[batch_idx, grid_y, grid_x]
        widths = pred_w[batch_idx, grid_y, grid_x]
        heights = pred_h[batch_idx, grid_y, grid_x]
        scores = pred_conf[batch_idx, grid_y, grid_x]
...
def boxes_to_midi_notes(boxes: torch.Tensor, total_time_sec: float) -> list:
    """Converts decoded YOLOX boxes back to MIR-eval friendly note dictionaries."""
    notes = []
    for box in boxes:
